...
public class BlogUserDAOImpl extends UserDAOImpl implements BlogUserDAO { 
	...
	private static final String LOAD_QUERY = "SELECT "+SELECT_USER_VALUES+" FROM TB_BLOG_USER WHERE NM_USER_ID = ? ";

	private static final String DELETE_QUERY = "DELETE FROM TB_BLOG_USER WHERE NM_USER_ID = ?";
	
	private static final String LIST_USERS_QUERY = "SELECT "+SELECT_USER_VALUES+" FROM TB_BLOG_USER WHERE NM_STATUS = ? ";
	
	private static final String LIST_USERS_GROUP_QUERY = "SELECT "+SELECT_USER_VALUES+" FROM TB_BLOG_USER WHERE NM_STATUS = ? " +
			"AND NM_GROUP_ID_FK = ?";
	
	private static final String LIST_USERS_GROUP_ForBlog_QUERY ="SELECT "+SELECT_USER_VALUES+" FROM TB_BLOG_USER WHERE NM_STATUS = ? " +
	" AND NM_GROUP_ID_FK = ? AND NM_USER_ID NOT IN (SELECT NM_USER_ID_FK FROM TB_BLOG)";

		
	private static final String REMIND_PASSWORD_QUERY = "SELECT "+SELECT_USER_VALUES+" FROM TB_BLOG_USER WHERE VC_EMAIL = ? ";
	...
	{
		return (BlogUser) new UserDAOImpl().login(login ,password);
	}
	...
	public Collection listUsersByGroupForBlog(int status,int groupId) throws SQLException,Exception {
		...
		try {
			...
			if (status == Constants.STATUS_ALL) {
				ps = conn.prepareStatement(LIST_USERS_GROUP_ForBlog_QUERY.replaceAll(
						"NM_STATUS = ", "NM_STATUS > "));
			} else {
