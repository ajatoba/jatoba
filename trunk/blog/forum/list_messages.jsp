...
<br/><br/><font style="font-size:11px; font-weight:bold; color:red; font-family:Trebuchet MS;"><bean:write name="mensagem_erro"/></font>
</logic:present> 

<logic:present name="message_sucesso">
<br/><br/><font style="font-size:11px; font-weight:bold; color:red; font-family:Trebuchet MS;"><bean:write name="message_sucesso"/></font>
</logic:present>  	

<logic:present name="no_messages">
<br/><br/><font style="font-size:11px; font-weight:bold; color:red; font-family:Trebuchet MS;"><bean:write name="no_messages"/></font>
</logic:present> 
</center>

	<div class="coment"><a href="/blog/message.do?act=addForm&topic=${topic.id}">Comentar</a></div>
	<div class="coment2"><a href="/blog/forum.do?act=list" >Voltar aos Tópicos</a></div>
</div>
</div>

<pg:pager url="message.do" maxIndexPages="5" maxPageItems="10">
...
<div id="6">
	...
	<p class="resp_tit"><i>
    	...
    	em ${m.insertDate.date}/${m.insertDate.month+1}/${m.insertDate.year+1900} <!--às ${m.insertDate.hours}:${m.insertDate.minutes}--></i></p>
	<p class="txt_resp"><b><bean:write name="m" property="title"/>: </b><bean:write name="m" property="content"/> </p>
</div>
</div>
</pg:item>
</logic:iterate>
		...
		<pg:index>
			<pg:prev>
			<a href="<%=pageUrl%>&act=list&topic=${m.topic.id}">[<< Anterior]</a>	
			</pg:prev>
				
			<pg:pages>
				<a href="<%= pageUrl %>&act=list&topic=${m.topic.id}"><%= pageNumber %></a>
			</pg:pages>
			
			<pg:next>
				<a href="<%= pageUrl%>&act=list&topic=${m.topic.id}">[Próxima >>]</a>
			</pg:next>
		</pg:index>
	</p>
	</pg:pager>	
