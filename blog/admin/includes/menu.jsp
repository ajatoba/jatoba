<link rel="stylesheet" href="/bloglog/admin/css/stylle.css" type="text/css">


<table id="Table_01" width="913px" border="0" cellpadding="0" cellspacing="0">
<tr>
		<td>
			<img id="header_01" src="/bloglog/admin/img/header_01.jpg" width="228" height="93" alt="" /></td>
		<td>
			<img id="header_02" src="/bloglog/admin/img/header_02.jpg" width="229" height="93" alt="" /></td>
		<td>
			<img id="header_03" src="/bloglog/admin/img/header_03.jpg" width="228" height="93" alt="" /></td>
		<td>
			<a href="/blog/adminUser.do?act=logout"><img src="/bloglog/admin/img/header_04.jpg" alt="" name="header_04" width="228" height="93" border="0" id="header_04" /></a></td>
  </tr>
	<tr>
	  <td colspan="9" bgcolor="#E9F2F7">
      <div style="float:left; width:26px; height:45px;"></div>
		<div style="float:left"><a href="/blog/admin/blog.do?act=list"><img src="/bloglog/admin/img/btn_blogs_over.jpg" alt="" name="btn_blogs_over" width="86" height="45" border="0" id="btn_blogs_over" /></a></div>
        <div style="float:left"><a href="/blog/admin/blogUser.do?act=list"><img src="/bloglog/admin/img/btn_blogueiros.jpg" alt="" name="btn_blogueiros" width="128" height="45" border="0" id="btn_blogueiros" /></a></div>
        <div style="float:left"><a href="/blog/admin/userCommentator.do?act=listCommentador"><img src="/bloglog/admin/img/btn_users.jpg" alt="" name="btn_users" width="109" height="45" border="0" id="btn_users" /></a></div>
        <div style="float:left"><a href="/blog/admin/comment.do?act=searchAll">EXCLUSÃO DE COMENTÁRIOS</a></div>
        <div style="float:left; width:156px; height:45px;"></div>
        <div style="float:left; width:167px; height:45px;"></div>
        <div style="float:left; width:241px; height:45px;"></div>
      </td>
  </tr>
</table>

